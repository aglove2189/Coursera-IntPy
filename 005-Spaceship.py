# http://www.codeskulptor.org/#user26_Df2aMTD2NrgaUEI.py
...
class Ship:
    ...
    def shoot(self):
        ...
        vel = [self.vel[0] + forward[0] * 5, self.vel[1] + forward[1] * 5]
    ...
    def update(self):
        ...
        if self.thrust:
            self.thrust = angle_to_vector(self.angle)
            self.vel[0] += self.thrust[0] * 0.15
            self.vel[1] += self.thrust[1] * 0.15
...
def draw(canvas):
    ...
    canvas.draw_image(debris_image, center, size, (wtime + WIDTH / 2, HEIGHT / 2), (WIDTH, HEIGHT))
    canvas.draw_text("Lives: " + str(lives), (WIDTH / 4, 40), 40, "White")
    canvas.draw_text("Score: " + str(score), (WIDTH / 2, 40), 40, "White")
